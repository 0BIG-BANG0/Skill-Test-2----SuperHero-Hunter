...
async function fetchDataAsync(id){
	...
	.then((data) => {
		console.log('Data : ',data);
            ...
            <div class="left">
                <img src="${data['image']['url']}" alt="Superhero Image">
            </div>
            <div class="right">
                <h1>${data.name}</h1>
                ...
                <div class="ratings">
                    <p><span>Combat:</span> ${data['powerstats']['combat']}</p>
                    <p><span>Strength:</span> ${data['powerstats']['strength']}</p>
                    <p><span>Intelligence:</span> ${data['powerstats']['intelligence']}</p>
                    <p><span>Power:</span> ${data['powerstats']['power']}</p>
                    <p><span>Speed:</span> ${data['powerstats']['speed']}</p>
                </div>
                <button type="button" id="${data.id}" class="btn fav btn-outline-success">Favorite</button>
            </div>
        </div>`
			main.innerHTML += item;
